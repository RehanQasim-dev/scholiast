...
- **OS Selection Toolbar Suppression (Android)**:
  - Suppression is two-layered because WebView populates the selection menu asynchronously *after* the mode starts — a one-time clear loses to late repopulation. Primary: `onWindowStartingActionMode` / `onWindowStartingSupportActionMode` wrappers (framework + AppCompat, since WryActivity extends AppCompatActivity) strip the menu in `onPrepareActionMode` (runs after population, on every show/invalidate). Backup: `onActionModeStarted` / `onSupportActionModeStarted` clear once plus a posted re-clear. (WebView has no `setCustomSelectionActionModeCallback` — that method exists only on TextView; calling it on a WebView fails Kotlin compilation.) Selection + handles are preserved (Android 6+ skips empty floating menus). Selections inside inputs/textareas/contenteditables keep the system menu: `lib/selectionBridge.ts` classifies every `selectionchange` (shared `EDITABLE_SELECTOR` const) and reports via the `AndroidSelection` JS bridge; the live-page iframe reports synchronously from its own injected script (`getScholiastIframeScript`, covered by `iframeSelection.test.ts`) instead of relying on the parent postMessage round-trip.
