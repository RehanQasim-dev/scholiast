...
- **OS Selection Toolbar Suppression (Android)**:
  - `MainActivity.onActionModeStarted` clears the system floating menu for article selections so the in-app SwatchPopup is reachable; selection + handles are preserved (Android 6+ skips empty floating menus). Selections inside inputs/textareas/contenteditables keep the system menu: `lib/selectionBridge.ts` classifies every `selectionchange` and reports via the `AndroidSelection` JS bridge.
  - Do NOT return null from `onWindowStartingActionMode` (tried before — it only declines a custom mode; the default toolbar still builds) and do NOT `mode.finish()` (kills the selection).
...
- **Release APKs always ship the local-STT engine**: CI `tauri android build` runs with `--features local-stt` (same in `docs/guides/build-and-release.md`) — an engine-less release lets installed models report ready and fail every transcription.
